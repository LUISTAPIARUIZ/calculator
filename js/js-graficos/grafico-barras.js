...
btnGraficar.addEventListener("click", function(){
    ...
    if(contadorBtnGraficar===0){
        objetoChart=graficadora(arrayNombreColumna,arraryValorColumna,inputNombreGrafico.value,arrayNombreColumna.length)
        contadorBtnGraficar++;
    }else{
        objetoChart.destroy();
        objetoChart=graficadora(arrayNombreColumna,arraryValorColumna,inputNombreGrafico.value,arrayNombreColumna.length)
...
const graficadora = (nombres,valores,nombreGrafico,cantidadDatos)=>{
